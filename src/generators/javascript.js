...
export default function javascript(ScratchBlocks) {
  /**
   * JavaScript code generator.
   * @type {!ScratchBlocks.Generator}
   */
  ScratchBlocks.JavaScript = new ScratchBlocks.Generator('JavaScript');
  ...
  ScratchBlocks.JavaScript.init = function(workspace) {
    ...
    for (let i = 0; i < variables.length; i++) {
      defvars.push(ScratchBlocks.JavaScript.variableDB_.getName(variables[i].getId(),
    ...
    */
    ...
    if (defvars.length) {
      ScratchBlocks.JavaScript.definitions_['variables'] =
        'var ' + defvars.join(', ') + ';';
    }
  };
  ...
  ScratchBlocks.JavaScript.finish = function(code) {
    ...
    for (var name in ScratchBlocks.JavaScript.definitions_) {
      definitions.push(ScratchBlocks.JavaScript.definitions_[name]);
    }
    // Clean up temporary data.
    delete ScratchBlocks.JavaScript.definitions_;
    delete ScratchBlocks.JavaScript.functionNames_;
    ScratchBlocks.JavaScript.variableDB_.reset();
  ...
  ScratchBlocks.JavaScript.getAdjusted = function(
    ...
    var order = opt_order || ScratchBlocks.JavaScript.ORDER_NONE;
    ...
    if (ScratchBlocks.isNumber(at)) {
      ...
      if (delta > 0) {
        at = at + ' + ' + delta;
        innerOrder = ScratchBlocks.JavaScript.ORDER_ADDITION;
      } else if (delta < 0) {
        at = at + ' - ' + -delta;
        innerOrder = ScratchBlocks.JavaScript.ORDER_SUBTRACTION;
      }
      if (opt_negate) {
        if (delta) {
          at = '-(' + at + ')';
        ...
        innerOrder = ScratchBlocks.JavaScript.ORDER_UNARY_NEGATION;
      }
